        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <x-container-second-div>
                <div class="p-6">
                    ...
                    <form method="GET" action="{{ route('reports.weekly-schedules') }}"
                        ...
                        <div>
                            ...
                            <x-select-input name="academic_management_id" id="academic_management_id">
                                ...
                                @foreach ($academicManagements as $am)
                                    <option value="{{ $am->id }}"
                                        {{ request('academic_management_id') == $am->id ? 'selected' : '' }}>
                        ...
                        <div class="flex items-end gap-2">
                            <a href="{{ route('reports.weekly-schedules', array_merge(request()->all(), ['format' => 'pdf'])) }}"
                            ...
                            <a href="{{ route('reports.weekly-schedules.export', request()->all()) }}"
                ...
                <div class="p-6">
                                    ...
                                    @foreach ($scheduleData as $day => $assignments)
                                        @foreach ($assignments as $index => $assignment)
                                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                                @if ($index === 0)
                                                    <td rowspan="{{ count($assignments) }}"
                                                        class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100 bg-gray-50 dark:bg-gray-800 align-top border-r border-gray-200 dark:border-gray-700">
                                                        {{ $day }}
                                                    </td>
                                                @endif
                                                <td
                                                ...
                                                <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-300">
                                                    <div class="font-medium">
                                                        {{ $assignment->userSubject->subject->name }}</div>
                                                    <div class="text-gray-500 dark:text-gray-400">
                                                        {{ $assignment->userSubject->subject->code }}</div>
                                                </td>
                                                <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-300">
                                                ...
                                                <td
                                                    ...
                                                    <div class="text-gray-500 dark:text-gray-400 text-xs">
                                                        {{ $assignment->classroom->infrastructure->name }}</div>
                                                </td>
                                            </tr>
                                        @endforeach
                                    @endforeach
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <p class="mt-4 text-gray-500 dark:text-gray-400">No se encontraron horarios con los filtros
                                seleccionados.</p>
                        </div>
                    @endif
                </div>
